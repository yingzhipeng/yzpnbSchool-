        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          ...
          <div class="btn" style="position:absolute;right: 0;top: 6px;width: 40%;">
            <el-link
               ...
               type="button" @click="getCodeFun()" :value="codeTest" style="color: #00BB29;border: none;background-color: none">{{codeTest}}</el-link>
          </div>
        </el-form-item>
...
<script>
  ...
  import registerApi from '@/api/register/index.js'
  export default {
    layout: 'sign',
    data() {
      return {
    ...
    methods: {
      ...
      getCodeFun() {
        ...
        this.$refs.userForm.validateField('mobile', (errMsg) => {
          if (errMsg == '') {
            this.loading=true
            registerApi.shortMessage(this.params.mobile).then(res => {
              this.loading=false
              this.sending = false;
              this.timeDown();
            });
