...
<script>
  import banner from '~/api/banner/index.js'
export default {
  data () {
    return {
      swiperOption: {
  ...
  created() {
    ...
    methods:{
      ...
      initDataObj() {
        banner.getNewRecommended().then(response => {
          this.teacherList = response.data.data.newRecommended.teacherList
          this.courseList = response.data.data.newRecommended.courseList
        })
      }
    },
}
