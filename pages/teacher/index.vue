      ...
      <section class="c-sort-box unBr">
        <div>
          ...
          <article v-if="data.total>0" class="i-teacher-list">
              <ul class="of">
                  <li v-for="teacher in data.allLimitTeacher" :key="teacher.id">
...
export default {
  ...
  //error为调用的错误信息
  asyncData({ params, error }) {//异步调用，进入页面不会直接调用，我们请求的时候才会调用，但只会执行一次
    return teacher.getAllLimitTeacher(1,8).then(response => {
  ...
  methods: {
      gotoPage(page){
          teacher.getAllLimitTeacher(page, 8).then(response => {
